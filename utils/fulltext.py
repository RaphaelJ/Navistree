...
def fulltext(model, field):
    class FullTextQuerySet(models.QuerySet):
        def search(self, query):
            

    class FullTextManager(models.Manager):
        def get_query_set(self):
            return FullTextQuerySet(model)

        def search(self, query):
            return model.get_query_set().search(query)

    class Index:
        word = models.CharField()
        element = models.ForeignKey(model)
        
        objects = FullTextManager

    Index.__name__ =  "__{0}_fulltext".format(model.__name__)
        
    return Index
    
    for f in fields:
        setattr(model, "__"+f.__name__+
    fields.__name__
